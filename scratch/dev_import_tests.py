...
def analyze_singular_matrix(J, tolerance=1e-10):
    ...
    if issparse(J):
        row_norms = np.array(
            [np.linalg.norm(J_csr[i].toarray()) for i in range(n_rows)]
        )
        col_norms = np.array(
            [np.linalg.norm(J_csc[:, j].toarray()) for j in range(n_cols)]
        )
    else:
...
def print_analysis(results):
    ...
    print(
        f"Sparsity: {100 * (1 - results['nnz']/(results['shape'][0]*results['shape'][1])):.2f}%"
    )
    ...
    print(
        f"Column norms - Min: {results['col_norms'].min():.2e}, Max: {results['col_norms'].max():.2e}"
    )
...
def suggest_removals(J, tolerance=1e-10):
    ...
    if analysis["rank"] is not None and analysis["rank"] < min(J.shape):
        ...
        try:
            ...
            if len(qr_result) == 3:
                ...
                raise ValueError(
                    f"Expected 3 return values from QR, got {len(qr_result)}"
                )
                            ...
                            linear_dependencies.append(
                                {
                                    ...
                                    "depends_on": [i],
                                    "coefficients": [
                                        np.dot(col_i, col_j) / np.dot(col_i, col_i)
                                    ],
                                }
                            )
                            remove_cols.add(j)
    ...
    if linear_dependencies:
        ...
        for dep in linear_dependencies[:10]:  # Show first 10
            print(
                f"  Column {dep['column']} is a linear combination of columns {dep['depends_on']}"
            )
