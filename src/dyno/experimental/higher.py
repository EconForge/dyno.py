...
def expand_symmetric_indices(
    sparse_coo_dict: Dict[Tuple[int, ...], Any],
) -> Dict[Tuple[int, ...], Any]:
...
def sparse_coo_to_dense(
    sparse_coo_dict: Dict[Tuple[int, ...], Any], shape: Tuple[int, ...]
) -> Any:
...
class HigherOrderDerivatives:
    def __init__(self, model: DynoModel):
        self.model = model
        self.residuals, self.symbols, self.v_context = self._build_sympy_representation(
            model
        )
    ...
    def compute_sparse_hessian(self) -> Dict[Tuple[int, ...], sp.Expr]:
        ...
        for i, residual in enumerate(self.residuals):
            for j, k in itertools.combinations_with_replacement(
                range(len(self.symbols)), 2
            ):
    ...
    def compute_sparse_third_derivatives(self) -> Dict[Tuple[int, ...], sp.Expr]:
        ...
        for i, residual in enumerate(self.residuals):
            for j, k, l in itertools.combinations_with_replacement(
                range(len(self.symbols)), 3
            ):
                deriv = sp.diff(
                    residual, self.symbols[j], self.symbols[k], self.symbols[l]
                )
                if deriv != 0:
    ...
    def compute_sparse_derivatives(
        self, max_order: int = 3
    ) -> List[Dict[Tuple[int, ...], sp.Expr]]:
    ...
    def evaluate_at_steady_state(
        self, sparse_derivatives_list: List[Dict[Tuple[int, ...], sp.Expr]]
    ) -> List[Dict[Tuple[int, ...], Any]]:
        values = [
            self.model.symbolic.context["steady_states"].get(e, 0.0)
            for e in self.model.symbols["endogenous"]
        ] * 3 + [
            self.model.symbolic.context["steady_states"].get(e, 0.0)
            for e in self.model.symbols["exogenous"]
        ]
