...
def test_jacobian_consistency():
    ...
    res_sparse, J_sparse = model.deterministic_residuals_with_jacobian(
        v0_flat, sparsify=True
    )
    J_sparse_dense = J_sparse.toarray()
    print(f"   Shape: {J_sparse.shape}")
    print(
        f"   Sparsity: {J_sparse.nnz}/{n_vars**2} ({100*J_sparse.nnz/n_vars**2:.2f}%)"
    )

    # Method 2: Dense Jacobian (4D tensor assembled into matrix)
    print("\n2. Computing dense Jacobian (non-sparse assembly)...")
    res_dense, J_dense = model.deterministic_residuals_with_jacobian(
        v0_flat, sparsify=False
    )
    ...
    res_4d, DD_4d = model.deterministic_residuals_with_jacobian(
        v0.reshape((T + 1, p)), sparsify=False
    )
    ...
    print(
        f"  Are they equal? {np.allclose(J_sparse_dense, J_dense, rtol=1e-12, atol=1e-14)}"
    )
    ...
    if not np.allclose(J_sparse_dense, J_dense, rtol=1e-12, atol=1e-14):
        ...
        for idx in largest_diffs:
            ...
            print(
                f"    [{i:3d},{j:3d}]: sparse={J_sparse_dense[i,j]:10.6f}, dense={J_dense[i,j]:10.6f}, diff={diff_sparse_dense[i,j]:10.6f}"
            )
    ...
    print(
        f"  Are they close? {np.allclose(J_sparse_dense, J_numerical, rtol=1e-5, atol=1e-7)}"
    )
    ...
    for idx in largest_num_diffs:
        ...
        var_col = j % p
        print(
            f"    [{i:3d},{j:3d}] (t_row={t_row:2d}, t_col={t_col:2d}, var_row={var_row}, var_col={var_col}):"
        )
        print(
            f"      analytical={J_sparse_dense[i,j]:10.6f}, numerical={J_numerical[i,j]:10.6f}, diff={diff_sparse_num[i,j]:10.6f}"
        )
    ...
    print(
        f"  Are they close? {np.allclose(J_dense, J_numerical, rtol=1e-5, atol=1e-7)}"
    )

    # Check residuals consistency
    print(f"\nResiduals consistency:")
    print(
        f"  Sparse method residuals match dense method? {np.allclose(res_sparse, res_dense)}"
    )
    print(
        f"  Max residual diff (sparse vs dense): {np.max(np.abs(res_sparse - res_dense)):.2e}"
    )

    # Assertions
    assert np.allclose(
        J_sparse_dense, J_dense, rtol=1e-12, atol=1e-14
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Jacobian sparsity: {J.nnz} non-zero elements out of {J.shape[0] * J.shape[1]}"
        )
